...
const Pricing: React.FC = () => (
  <div className="max-w-5xl mx-auto py-16 px-4">
    ...
    <div className="grid md:grid-cols-3 gap-8 mb-12">
      {plans.map((plan, idx) => (
        <div key={plan.name} className="bg-card/60 rounded-xl shadow-md border border-border/40 p-8 flex flex-col items-center">
          {plan.name === "Bronze" && <Star className="w-8 h-8 text-primary mb-2" />}
          {plan.name === "Premium" && <Crown className="w-8 h-8 text-accent mb-2" />}
          {plan.name === "Diamond" && <Zap className="w-8 h-8 text-success mb-2" />}
          ...
          <Button asChild variant={plan.name === "Premium" ? "premium" : "outline"} size="lg" className="w-full">
    ...
    <div className="text-center mt-8">
      ...
      <Button asChild size="lg" variant="premium" className="mr-4">
